...
func find_bombs():
	if !color_bomb_used:
		# Iterate over current matches array
...
func make_bomb(bomb_type, color):
	...
	for i in current_matches.size():
		...
		if all_pieces[current_column][current_row] == piece_1 and piece_1.color == color:
			...
			emit_signal("check_goal", piece_1.color)
		...
		if all_pieces[current_column][current_row] == piece_2 and piece_2.color == color:
			# Make piece_2 bomb
			damage_special(current_column, current_row)
			emit_signal("check_goal", piece_2.color)
			...
			if all_pieces[i][j] != null and !is_piece_sinker(i, j):
				if all_pieces[i][j].color == color:
					...
					if all_pieces[i][j].is_row_bomb:
						match_all_in_row(j)
					if all_pieces[i][j].is_color_bomb:
						find_adjacent_pieces(i, j)
...
func generate_slime():
	# Make sure there are slime pieces on board
	if slime_spaces.size() > 0:
		...
		while !slime_made and tracker < 100:
			...
			if neighbour != null:
				# Turn neighbour into a slime
				# Remove that piece
				all_pieces[neighbour.x][neighbour.y].queue_free()
				# Set it to null
				all_pieces[neighbour.x][neighbour.y] = null
				# Add this new spot to the array of slime
				slime_spaces.append(Vector2(neighbour.x, neighbour.x))
				# Send signal to slime_holder to make new slime
